...
$sql = 'SELECT     B.id, A.name, B.fromdate, B.todate, (SELECT COUNT(*) FROM {tool_gnotify_tpl_ins_ack} C WHERE B.id=C.insid) ack
...
foreach ($instemplates as $value) {
    ...
    array_push($readytpl, ['id' => $value->id, 'name' => $value->name, 'fromdate' => $fromdate, 'todate' => $todate, 'ack' => $value->ack]);
}
